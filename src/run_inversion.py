...
from gen_videos2 import gen_interp_video
...
def space_regularizer_loss(
    ...
    for w_code in territory_indicator_ws:
        new_img = G_pti.synthesis(w_code, noise_mode='none', force_fp32=True)['image']
        with torch.no_grad():
            old_img = G_original.synthesis(w_code, noise_mode='none', force_fp32=True)['image']
...
def pivotal_tuning(
    ...
    for step in tqdm(range(num_steps)):
        ...
        synth_images = G_pti.synthesis(w_pivot[0].repeat(1,G.num_ws,1), noise_mode=noise_mode)['image']
        # loss = torch.sum(synth_images)
        # track images
...
def project(
    ...
    *,
    ...
    # Load VGG16 feature detector.
    print("Load VGG16 feature detector.")
    vgg16_url = 'https://api.ngc.nvidia.com/v2/models/nvidia/research/stylegan3/versions/1/files/metrics/vgg16.pkl'
    vgg16 = metric_utils.get_feature_detector(vgg16_url, device=device)

    # Features for target image.
    print("Features for target image.")
    target_images = target.unsqueeze(0).to(device).to(torch.float32)
    if target_images.shape[2] > 256:
        target_images = F.interpolate(target_images, size=(256, 256), mode='area')
    target_features = vgg16(target_images, resize_images=False, return_lpips=True)
    ...
    for step in tqdm(range(num_steps)):
        ...
        # Synth images from opt_w.
        synth_images = G.synthesis(w_opt[0].repeat(1,G.num_ws,1), noise_mode=noise_mode)['image']
...
def run_projection(
    ...
    python run_inversion.py \
        --outdir=/storage2/guangrun/qijia_3d_model/eg3d/large_gen_imagenet_crop224_sz128/00010-shapenet-imagenet_train_crop224_sz128-gpus8-batch64-gamma0.3/ \
        --network=/storage2/guangrun/qijia_3d_model/eg3d/large_gen_imagenet_crop224_sz128/00010-shapenet-imagenet_train_crop224_sz128-gpus8-batch64-gamma0.3/network-snapshot-020966.pkl \
        --target=/scratch/local/ssd/guangrun/qijia_3d_model/DPT/input/test.png
    """
    np.random.seed(seed)
    ...
    synth_image = G.synthesis(projected_w.repeat(1, G.num_ws, 1))['image']
    ...
    gen_interp_video(G=G, mp4=f"{outdir}/inv.mp4", ws=projected_w.unsqueeze(0), bitrate='10M', grid_dims=(1,1), num_keyframes=1, w_frames=120, seeds=[0], cfg='Shapenet', image_mode='image', gen_shapes=True)
