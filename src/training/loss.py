...
class StyleGAN2DepthLoss(Loss):
    """
    Loss function for StyleGAN2 with additional supervision on depth."""
    def __init__(self, device, G, D, augment_pipe=None, r1_gamma=10, style_mixing_prob=0, pl_weight=0, 
                 ...
                 r1_gamma_init=0, r1_gamma_fade_kimg=0, neural_rendering_resolution_initial=64, neural_rendering_resolution_final=None, 
        ...
        self.resample_filter = upfirdn2d.setup_filter([1,3,3,1], device=device)
        self.blur_raw_target = True
        self.depth_loss = DepthLoss(device=device)
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                training_stats.report('Loss/G/loss', loss_Gmain)
                # calculate depth loss
                depth_loss = self.depth_loss.calculate_loss(gen_img['image_raw'], gen_img['image_depth'])
                training_stats.report('Loss/depth/loss', depth_loss)
            with torch.autograd.profiler.record_function('Gmain_backward'):
                (loss_Gmain.mean() + self.depth_loss_weight*depth_loss).mul(gain).backward()
