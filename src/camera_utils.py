...
class RandomCameraPoseSampler:
    """
    Samples random camera poses from a uniform distribution over the unit sphere.
    """

    @staticmethod
    def sample(batch_size=1, device='cpu'):
        camera_origins = torch.randn((batch_size, 3), device=device)
        forward_vectors = math_utils.normalize_vecs(-camera_origins)
        return create_cam2world_matrix(forward_vectors, camera_origins)

class Zero123PlusPlusCameraPoseSampler:
    """
    Samples camera poses following the Zero123++ method. 
    Elevation angle is between +30 and -20 deg, azimuthal angle is increased every 60 degrees.
    """
    @staticmethod
    def sample(batch_size=1, device='cpu'):
        azimuthal_angles = torch.tensor([0, 60, 120, 180, 240, 300], dtype=torch.float, device=device)
        elevation_angles = torch.tensor([30, 10, -10, -30], dtype=torch.float, device=device)

        azimuthal_angles = azimuthal_angles.unsqueeze(0).expand(batch_size, -1)
        elevation_angles = elevation_angles.unsqueeze(0).expand(batch_size, -1)

        azimuthal_angles = azimuthal_angles + torch.rand((batch_size, 1), device=device) * 60
        elevation_angles = elevation_angles + torch.rand((batch_size, 1), device=device) * 20

        azimuthal_angles = azimuthal_angles * 3.14159 / 180
        elevation_angles = elevation_angles * 3.14159 / 180

        azimuthal_angles = azimuthal_angles % (2 * 3.14159)
        elevation_angles = torch.clamp(elevation_angles, -20 * 3.14159 / 180, 30 * 3.14159 / 180)

        theta = azimuthal_angles
        phi = elevation_angles

        camera_origins = torch.zeros((batch_size, 3), device=device)

        camera_origins[:, 0:1] = torch.sin(phi) * torch.cos(math.pi-theta)
        camera_origins[:, 2:3] = torch.sin(phi) * torch.sin(math.pi-theta)
        camera_origins[:, 1:2] = torch.cos(phi)
