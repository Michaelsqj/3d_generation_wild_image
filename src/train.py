...
def init_dataset_kwargs(data, class_name='training.dataset.ImageFolderDataset', **kwargs):
    try:
        ...
        dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs, **kwargs) # Subclass of training.dataset.Dataset.
...
@click.option('--data_class',   help='Class name for the dataset', metavar='STR',               type=str, default='training.dataset.ImageFolderDataset', show_default=True)
@click.option('--gpus',         help='Number of GPUs to use', metavar='INT',                    type=click.IntRange(min=1), required=True)
@click.option('--batch',        help='Total batch size', metavar='INT',                         type=click.IntRange(min=1), required=True)
@click.option('--gamma',        help='R1 regularization weight', metavar='FLOAT',               type=click.FloatRange(min=0), required=True)
@click.option('--loss_class',   help='Class name for the loss', metavar='STR',                  type=str, default='training.loss.StyleGAN2Loss', show_default=True)
@click.option('--depth_loss_weight', help='Weight for depth loss', metavar='FLOAT',              type=click.FloatRange(min=0), default=1, show_default=True)
...
def main(**kwargs):
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name=opts.loss_class)
    if opts.loss_class == 'training.loss.StyleGAN2DepthLoss':
        c.loss_kwargs.depth_loss_weight = opts.depth_loss_weight
    c.data_loader_kwargs = dnnlib.EasyDict(pin_memory=True, prefetch_factor=2)

    # Training set.
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, class_name=opts.data_class)
