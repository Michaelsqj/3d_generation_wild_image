...
from training.triplane_vid import TriPlaneGenerator
...
def gen_interp_video(G, mp4: str, ws=None, seeds=[0], shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    cls = cls_list[index]
    cls = 437
    class_condition = torch.zeros(len(all_seeds), G.c_dim, device=device)
    class_condition[:, cls] = 1


    camera_lookat_point = torch.tensor([0,0,0], device=device)
    zs = torch.from_numpy(np.stack([np.random.RandomState(seed).randn(G.z_dim) for seed in all_seeds])).to(device)
    cam2world_pose = LookAtPoseSampler.sample(3.14/2, 3.14/2, camera_lookat_point, radius=1.2, device=device)
    ...
    c = c.repeat(len(zs), 1)
    if ws is None:
        ws = G.mapping(z=zs, c=class_condition, truncation_psi=psi, truncation_cutoff=truncation_cutoff)
    else:
    ...
    if gen_shapes:
        ...
        if output_ply:
            ...
            convert_sdf_samples_to_ply(np.transpose(sigmas, (2, 1, 0)), [0, 0, 0], 1, mp4.replace('mp4','mrc'), level=10)
        else: # output mrc
...
@click.option('--cfg', help='Config', type=click.Choice(['FFHQ', 'AFHQ', 'Shapenet']), required=False, metavar='STR', default='Shapenet', show_default=True)
...
def generate_images(
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        ...
        G = dill.load(f)['G_ema'].to(device) # type: ignore
    ...
    seeds_ = [36]
    output = os.path.join(outdir, f'{seeds_[0]}.mp4')

    # ws = torch.tensor(np.load('/storage2/guangrun/qijia_3d_model/eg3d/large_gen_imagenet_crop224_sz128/00010-shapenet-imagenet_train_crop224_sz128-gpus8-batch64-gamma0.3/projected_w.npz')['w']).to(device)
    # gen_interp_video(G=G, mp4=output, ws=ws, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, seeds=seeds_, shuffle_seed=shuffle_seed, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gen_shapes=shapes)
    gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, seeds=seeds_, shuffle_seed=shuffle_seed, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gen_shapes=shapes)
